...
def domainResolveOnPlesk(domainName):
    addr = socket.gethostbyname(domainName)
    sys.stdout.write("Check if [ " + domainName + " ] resolves on Plesk : ")
...
for line in vhostsList:
    ...
    # Check if Wordpress instance is running
    if not domainResolveOnPlesk(fqdn):
        continue

    if not hasActiveWpInstance():
        continue
    ...
    wpInstallPath = hasActiveWpInstance()
    if wpInstallPath == False:
        continue
